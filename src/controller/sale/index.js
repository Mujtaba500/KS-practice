  ...
  create: async (req, res) => {
    try {
      ...
      for (let i = 0; i < saleProducts.length; i++) {
        ...
        if (product.stock < saleProducts[i].productQuantity) {
          return res.status(400).json({
            message: `The product with name ${product.name} has insufficient stock`,
          });
        }
        product.stock -= saleProducts[i].productQuantity;
        await product.save();
